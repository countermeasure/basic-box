...
_test_alternative() {
  if update-alternatives --display "$1" | grep -E "$2 - priority $3" &>/dev/null; then
...
_test_gsettings() {
  if [[ $(gsettings get org."$1" "$2") == "$3" ]]; then
    echo -e "${ansi_green}🗸 ${ansi_clear}GNOME setting ${1} ${2} is ${3}"
  else
    echo -e "${ansi_red}✗ ${ansi_clear}GNOME setting ${1} ${2} is not ${3}"
  fi
}
...
_test_python_package_is_installed() {
  if pipx list | grep -E "package $1 .+ installed" &>/dev/null; then
...
_test_service_is_active() {
  if sudo "$1" status | grep 'Status: active' &>/dev/null; then
    echo -e "${ansi_green}🗸 ${ansi_clear}${1} is active"
  else
    echo -e "${ansi_red}✗ ${ansi_clear}${1} is not active"
  fi
}
...
test() {
  ...
  _test_file_exists /usr/local/share/man/man1/yt-dlp.1
...
%Creset%C(magenta)%an %C(cyan)%ar %C(dim white)on %ad %C(auto)%d"
  ...
  _test_file_exists /usr/local/share/man/man1/trash.1
  _test_file_exists /usr/local/share/man/man1/trash-empty.1
  _test_file_exists /usr/local/share/man/man1/trash-list.1
  _test_file_exists /usr/local/share/man/man1/trash-put.1
  _test_file_exists /usr/local/share/man/man1/trash-restore.1
  _test_file_exists /usr/local/share/man/man1/trash-rm.1
  ...
  _test_file_exists /usr/local/share/man/man1/nvim.1
  ...
  _test_file_exists /usr/local/share/man/man1/bandwhich.1
